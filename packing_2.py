...
def solve_rectangular_arrangement(container_w, container_h, tile_w, tile_h, max_tiles):
    """Try to solve with perfect rectangular arrangements first."""
    print(f"🎯 TRYING RECTANGULAR ARRANGEMENTS...")
    
    # Find possible rectangular arrangements
    arrangements = []
    for rows in range(1, max_tiles + 1):
        for cols in range(1, max_tiles // rows + 1):
            if rows * cols <= max_tiles:
                total_w = cols * tile_w
                total_h = rows * tile_h
                if total_w <= container_w and total_h <= container_h:
                    # Calculate centering position
                    start_x = (container_w - total_w) // 2
                    start_y = (container_h - total_h) // 2
                    arrangements.append((rows, cols, start_x, start_y, rows * cols))
    
    # Sort by number of tiles (descending)
    arrangements.sort(key=lambda x: x[4], reverse=True)
    
    if arrangements:
        rows, cols, start_x, start_y, total_tiles = arrangements[0]
        print(f"   ✅ Best rectangular: {rows}×{cols} grid = {total_tiles} tiles")
        ...
        for r in range(rows):
            for c in range(cols):
                x = start_x + c * tile_w
                y = start_y + r * tile_h
                positions.append((x, y, tile_w, tile_h, "original"))
        
        return total_tiles, positions
...
def solve_rectangle_packing_optimized(container_w, container_h, tile_w, tile_h, 
    ...
    """
    ENHANCED VERSION: Prioritizes compact, centered arrangements with no gaps.
    Maintains same interface as original function.
    """
    print(f"🔧 ENHANCED COMPACT SOLVER: {tile_w}×{tile_h} tiles in {container_w}×{container_h} container")
    ...
    # STRATEGY 1: Try perfect rectangular arrangements first
    rect_tiles, rect_positions = solve_rectangular_arrangement(container_w, container_h, tile_w, tile_h, max_tiles)
    
    if rect_tiles >= max_tiles * 0.8:  # If rectangular solution is good enough (80%+ of target)
        print(f"✅ PERFECT RECTANGULAR SOLUTION: {rect_tiles} tiles")
        solve_info = {
            'status': pywraplp.Solver.OPTIMAL,
            'solve_time': 0.1,
            'variables': 0,
            'constraints': 0,
            'objective_value': rect_tiles,
            'method': 'rectangular'
        }
        return rect_tiles, rect_positions, solve_info
    
    # STRATEGY 2: Enhanced ILP with compactness optimization
    print(f"🚀 FALLING BACK TO ENHANCED ILP...")
    ...
    # Calculate center coordinates for compactness
            ...
            for k in range(container_h):
                for o, (w, h) in enumerate(orientations):
                    ...
                    if j + w <= container_w and k + h <= container_h:
                        # Enhanced symmetry breaking for first tile
                        if i == 0:
                            # Force first tile closer to center
                            if j > container_w // 2 or k > container_h // 2:
                                continue
    ...
    # 1. Each tile placed at most once
    for i in range(max_tiles):
    ...
    # === ENHANCED OBJECTIVE WITH COMPACTNESS ===
    objective = solver.Objective()
    compactness_weight = 0.1  # Adjustable parameter
            ...
            for k in range(container_h):
                for o in range(len(orientations)):
                    if (i, j, k, o) in x:
                        ...
                        # Compactness bonus: prefer positions closer to center
                        tile_center_x = j + orientations[o][0] / 2
                        tile_center_y = k + orientations[o][1] / 2
                        distance_from_center = abs(tile_center_x - center_x) + abs(tile_center_y - center_y)
                        compactness_bonus = compactness_weight * (50 - distance_from_center)
                        
                        # Edge penalty: discourage positions near container edges
                        edge_distance = min(j, k, container_w - j - orientations[o][0], container_h - k - orientations[o][1])
                        edge_bonus = compactness_weight * 0.5 * edge_distance
                        
                        final_coefficient = base_reward + compactness_bonus + edge_bonus
                        objective.SetCoefficient(x[i, j, k, o], final_coefficient)
    
    objective.SetMaximization()
    
    # === SOLVE ===
    print(f"🚀 Starting enhanced solver...")
    ...
    if status in [pywraplp.Solver.OPTIMAL, pywraplp.Solver.FEASIBLE]:
        ...
        print(f"🎯 Tiles placed: {int(solver.Objective().Value()) // 1000}")  # Divide by base_reward
        ...
        return len(tile_positions), tile_positions, solve_info
...
def test_optimized_solver():
    ...
    test_cases = [
        (40, 48, 24, 16, "Pinwheel Test (Optimized)"),
